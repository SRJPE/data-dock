...
plot_test <- plot_test |> 
  group_by(sample_day, location_name, field_run_type_id) |> 
  ...
  mutate(sample_day = case_when(is.na(sample_day) ~ 0, # fill NAs with zeros
                                TRUE ~ sample_day)) |> glimpse()
  
ggplot(plot_test |> filter(location_name == "Sac-KNL"), aes(x = sample_day, y = count, color = field_run_type_id)) +
  geom_col() +
...
```{r echo=FALSE}
ggplot(plot_test |> filter(location_name == "Sac-KNL"), aes(x = sample_day, color = field_run_type_id, fill = field_run_type_id)) +
...
```{r echo=FALSE}
ggplot(plot_test |> filter(location_name == "Clear"), aes(x = sample_day, color = field_run_type_id, fill = field_run_type_id)) +
  geom_density(alpha = 0.3, adjust = 1.5) +
  facet_wrap(~ field_run_type_id) +
  theme_minimal() +
  labs(x = "Sample Day", y = "", fill = "Run Type", color = "Run Type")
