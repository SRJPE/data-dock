...
```{r echo=FALSE}
# Same plot than above but ploty 
bar_plot_2 <- ggplot(run_summary_month_site |> filter(location_name == "Mill"), aes(x = month, y = run_percent, fill = field_run_type_id)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~ location_name + year, ncol = 1) +
  scale_y_continuous(limits = c(0, 100)) +
  scale_fill_manual(values = tol_muted) +
...
line_plot_month <- ggplot(run_summary_complete |> filter(location_name == "Battle"), aes(x = month, y = run_percent, color = field_run_type_id, group = field_run_type_id)) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = tol_muted) +
  scale_y_continuous(limits = c(0, 100),
...
```{r echo=FALSE}
# line_plot_month_2 <- ggplot(run_summary_complete_2 |> filter(location_name == "Mill"), aes(x = month, y = run_percent, color = field_run_type_id, group = field_run_type_id)) +
#   geom_line() +
#   geom_point() +
#   scale_color_manual(values = tol_muted) +
#   scale_y_continuous(limits = c(0, 100),
...
```{r echo=FALSE}
# using data object that assigns NA when no run is present at all
line_plot_month_2 <- ggplot(run_summary_complete |> filter(location_name == "Mill"), aes(x = month, y = run_percent, color = field_run_type_id, group = field_run_type_id)) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = tol_muted) +
  scale_y_continuous(limits = c(0, 100),
...
run_summary_complete_by_year <- run_designation |>  
  group_by(year, location_name, field_run_type_id) |>
  summarise(total_samples = n(), .groups = "drop") |>
...
line_plot_year_grouped <- ggplot(run_summary_complete_by_year |> filter(location_name == "Battle"), aes(x = year, y = run_percent_year, color = field_run_type_id, group = field_run_type_id)) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = tol_muted) +
  scale_y_continuous(limits = c(0, 100),
...
#   filter(location_name == "Battle"), aes(x = date, y = run_percent, color = field_run_type_id, group = field_run_type_id)) +
#   geom_line() +
#   geom_point() +
#   scale_color_manual(values = tol_muted) +
#   scale_y_continuous(limits = c(0, 100),
...
```{r echo=FALSE}
line_plot_year_grouped_2 <- ggplot(run_summary_complete_by_year |> filter(location_name == "Mill"), aes(x = year, y = run_percent_year, color = field_run_type_id, group = field_run_type_id)) +
  geom_line() +
  geom_point() +
  scale_color_manual(values = tol_muted) +
  scale_y_continuous(limits = c(0, 100),
...
plot_test <- plot_test |> 
  group_by(day_standard, location_name, field_run_type_id) |> 
  summarise(count = n(), .groups = "drop") |>
  mutate(total_sample_day = sum(count)) |> 
         # run_percent = (count / total_sample_day) * 100) |>
  ungroup() |> 
  # mutate(sample_day = case_when(is.na(sample_day) ~ 0, # fill NAs with zeros
  #                               TRUE ~ sample_day)) |> 
  glimpse()
  
ggplot(plot_test |> filter(location_name == "Sac-KNL"), aes(x = day_standard, y = count, color = field_run_type_id)) +
  geom_col() +
  facet_wrap(~ field_run_type_id) +
  theme_minimal()
```
...
dist_1 <- ggplot(plot_test |> filter(location_name == "Sac-KNL"), aes(x = day_standard, color = field_run_type_id, fill = field_run_type_id)) +
  geom_density(alpha = 0.3, adjust = 1.5) +
  facet_wrap(~ field_run_type_id, ncol = 1) +
  theme_minimal() +
...
ggplot(plot_test |> filter(location_name == "Clear"), aes(x = day_standard, color = field_run_type_id, fill = field_run_type_id)) +
  geom_density(alpha = 0.3, adjust = 1.5) +
  facet_wrap(~ field_run_type_id) +
  theme_minimal() +
  labs(x = "Sample Day", y = "", fill = "Run Type", color = "Run Type")
...
```{r echo=FALSE, warning=FALSE}
dist_2 <- ggplot(plot_test |> filter(location_name == "Clear"),
       aes(x = day_standard, color = field_run_type_id, fill = field_run_type_id)) +
  geom_density(alpha = 0.3, adjust = 1.5) +
  facet_wrap(~ field_run_type_id, ncol = 1) +
  theme_minimal() +
  scale_x_continuous(name = "Month", # modify x label to months 
