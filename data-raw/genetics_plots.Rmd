...
bar_plot <- ggplot(run_summary_month_site |> filter(location_name == "Battle"), aes(x = month, y = run_percent, fill = run_name)) +
  geom_bar(stat = "identity", position = "stack") +
...
```{r echo=FALSE}
# Same plot than above but ploty 
bar_plot_2 <- ggplot(run_summary_month_site |> filter(location_name == "Mill"), aes(x = month, y = run_percent, fill = run_name)) +
  geom_bar(stat = "identity", position = "stack") +
  facet_wrap(~ location_name + year, ncol = 1) +
  scale_y_continuous(limits = c(0, 100)) +
  scale_fill_manual(values = tol_muted) +
...
dist_1 <- ggplot(plot_test_clean |> filter(location_name == "Sac-KNL"), aes(x = day_standard, color = run_name, fill = run_name)) +
...
Same plot than above but removing Early/Late Heterozygous
