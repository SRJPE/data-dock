...
ggplot(mode_per_month, aes(x = month, y = station_id, fill = value)) +
  geom_tile() +
  facet_wrap(~ analyte, ncol = 1, scales = "free_y") +
  scale_fill_brewer(palette = "Set3", na.translate = FALSE) +
  labs(
    title = "Modal Category by Station and Month",
    x = "Month",
    y = "Station",
    fill = "Most Frequent") +
  theme_minimal() +
...
#TODO draft another heatmap plot:
# x days of year, Y-axis year. Facet by station 
# color pallet to be "continuous"
  
```


Below is finer resolution/granularity. however it is challanging to see all data

```{r echo=FALSE}
p_heatmap_2 <- ggplot(wq_chr, aes(x = date, y = station_id, fill = value)) +
  geom_tile() +
  facet_wrap(~ analyte, ncol = 1, scales = "free_y") +
  scale_fill_brewer(palette = "Set3", na.translate = FALSE) +
...
ggplotly(p_heatmap_2, tooltip = c("x", "y", "fill"))
```


Plots for Non detect samples 
